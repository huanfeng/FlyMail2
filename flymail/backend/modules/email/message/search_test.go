...
func TestSearchMessages(t *testing.T) {
	...
	// 主题命中（中文）
	if got, _ := repo.SearchMessages("纪要", nil, 0, 50, message.Filter{}); len(got) != 1 || got[0].Subject != "周会纪要" {
		t.Errorf("主题搜索失败: %+v", got)
	}
	// 发件人命中
	if got, _ := repo.SearchMessages("alice", nil, 0, 50, message.Filter{}); len(got) != 1 || got[0].FromAddr != "alice@x.com" {
		t.Errorf("发件人搜索失败: %+v", got)
	}
	// 跨账户命中 + 大小写不敏感（subject "Invoice" 与 snippet "invoice"）
	got, _ := repo.SearchMessages("invoice", nil, 0, 50, message.Filter{})
	if len(got) != 2 {
...
func TestSearchMessagesEscapesWildcards(t *testing.T) {
	...
	if got, _ := repo.SearchMessages("100%", nil, 0, 50, message.Filter{}); len(got) != 1 || got[0].Subject != "100% done" {
