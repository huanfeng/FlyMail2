...
//
// ⚠ 偏移量必须写成 Z07:00 而不是 -07:00。
//
// Go 的 -07:00 只认字面的 ±hh:mm，遇到 UTC 的 `Z` 会解析失败；Z07:00 两种都认。
// 这不是理论问题：2026-09-16 那次「邮件日期统一存 UTC」的迁移
// （EnsureUTCDates 用 strftime('%Y-%m-%dT%H:%M:%SZ', date)）把全库日期都写成了
// Z 结尾，而这里的布局没跟着改，于是 parseDBTime 对**每一行**都返回零值。
//
// 后果有两层，第二层更隐蔽：
//  1. 会话列表的日期显示成 0001-01-01（界面上是「1901年1月」之类的怪年份）
//  2. LastDate 同时是**游标键**——零值会让下一页查询立刻为空，
//     等于会话视图的分页悄悄失效，滚到底再也加载不出更多
