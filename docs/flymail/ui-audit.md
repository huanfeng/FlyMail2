...
共发现 41 条，**至 2026-09-13 已全部处理**（六轮）。
第一轮 16 条，第二轮 3 条，第三轮 3 条，做浏览器通知时顺带修掉第 3、27 条，
第四轮 4 条（第 11、12、23、24 条），第五轮 3 条（第 7、9、13 条），
第六轮 14 条（其余全部）。

这份文档从此不再是待办清单，而是**六轮里每条缺陷的判据与做法的记录**——
其中反复出现的几种形状（见第八、九节）比条目本身更值得下次动 UI 前读一遍。
...
## 五、第五轮已处理（3 条）——长操作的可见性、原生 confirm、校验错误的位置

处理 P1 里剩下的第 7、9、13 条。做下去之后发现第 7 条根本不是「缺进度表达」，
而是**那段代码本来就是坏的**——下面第一小节。

### 7. 长操作没有进度表达

原判断只说对了一半。实现时发现三处真缺陷：

**(a) 轮询跟错了账户，而且停不下来。**
`useSyncStatus(accountId, syncEnabled)` 里的 `accountId` 是**当前选中**的账户，
不是被点同步的那个。在账户 A 上点账户 B 的同步：B 的图标不转（读的是 A 的状态），
而且 A 的 phase 既不是 done 也不是 error，那个把开关置回 false 的 effect 永远等不到条件，
**每秒一个请求一直发到切账户或刷新为止**。

**(b) 前端的 `SyncPhase` 漏了后端的 `'queued'`。**
后端 `service.go` 那行注释写得明明白白——「前端未识别按进行中展示」——
而前端的判据只认 `folders | messages`。用户按下同步、后端在排队等全局名额时，
屏幕上什么都不会发生。

**(c) 后端的 `total` / `processed` 一直都在，前端一行没用过。**

改法是把整套逻辑抽成 `src/hooks/useAccountSync.ts`，侧栏与设置面板的账户卡共用
（原先是各写一份、各踩各的坑，而且两份共用同一个查询键 `['sync-status', id]`，
还能互相看见对方写进缓存的东西）。侧栏账户行下方现在有进度条与
「正在同步邮件… 1234 / 4560」，`queued` 单独有文案。

刻意**不**显示百分比：`total` 是各文件夹边发现边累加的，百分比会往回跳；计数不会。

另外两块一并做了：

- **SSE 连接状态外露**。`connectRealtime` 加 `onState` 回调，`useRealtimeSync` 返回
  `{ offline }`。判据不是「此刻是不是 connecting」而是「连续 6 秒没连上」——
  重连很频繁（合盖唤醒、切网络、后端重启），照实显示只会让提示条不停闪。
- **附件上传进度**。10MB 附件此前是按钮变「发送中…」然后一动不动几十秒，与卡死无异。

### 9. 八处 window.confirm

做成 `useConfirm()` 返回 **Promise** 而不是「传 onConfirm 回调」：调用点因此保持
`if (!(await confirm({...}))) return` 的形状，与 `if (!window.confirm(...)) return`
逐字对应，八处改动都是机械的。

顺带发现 `settings.account.deleteConfirm` 和 `account.deleteConfirm` 是一字不差的两份，合并了。

### 13. 撰写器校验错误显示在会滚走的位置

移到与发送按钮同处的固定区域，容器**常驻**（live region 与内容一起插进 DOM 时读屏不播报）。

清理时发现 `infoMessage` 这份 state 从来没有被设成过非 null——在本轮之前就是死的。
我一开始把它和 `validationError` 合成了一个 `composeMsg`，让那段看起来能显示两类消息，
实际只可能显示一类。已删除。

### 复审抓出的四条

第五轮同样在提交前跑了一次独立 code-reviewer。四条全是真的，其中第一条是**我本轮引入的回归**。

#### S1（严重，本轮引入）同一个账户的第二次同步，界面上什么都不会发生

我把查询键从恒定的 `['sync-status', accountId]` 改成了随 `syncingAccountId` 变。
旧代码的键从不变化，`phase` 一直停在 `'done'`、依赖不翻转所以 effect 不重跑——
**纯属侥幸，但它侥幸对了**。改成随 id 变之后这个侥幸没了：

同步跑完 → 键变成 `['sync-status', null]` → `phase` 变 `undefined`。
再点同一个账户 → react-query **同步**返回缓存里那条 `'done'` → 依赖从 `undefined`
翻回 `'done'` → 终态判定当场触发 → 轮询根本没启动。

于是本轮新加的转圈、进度条、阶段计数，在每个账户的第二次及以后的同步里全部不出现。

修法：开轮询之前先 `setQueryData(['sync-status', id], { phase: 'queued' })`。
这不是乐观猜测——后端 `Service.Trigger` 在**返回 202 之前**就已经同步调用了
`status.begin(accountID, PhaseQueued)`，所以那一刻服务端确实就是 queued。

> **教训**：改动让一个原本恒定的值变成会变的，就要重新问一遍「谁在依赖它不变」。
> 旧代码能跑不等于旧代码有道理，而**我替换掉的恰好是那个让它能跑的偶然**。

#### S2 / S3（严重）手写浮层的键盘处理器与 radix 浮层打架

同一个根因的两种表现。radix 浮层走 Portal 挂在 `<body>` 末尾，**永远在手写浮层之外**：

- **Esc**：radix 处理完只调 `preventDefault()`，**不**调 `stopPropagation()`，
  事件照常冒泡到 `document`，设置面板那个「按 Esc 就 onClose」的监听器跟着触发。
  用户想取消一次删除，代价是整个设置面板消失。
- **Tab**：focus trap 的判据是「焦点不在我的 root 里就拉回来」，对 Portal 层恒成立，
  于是每一次 Tab 焦点都被拽回外层浮层——radix 浮层里的按钮**键盘完全够不着**。

**这两条不是确认框带来的。** AccountDialog / RuleDialog / ChannelDialog 早就是从设置面板
里弹出的 radix 浮层，它们一直都有这个毛病；`window.confirm` 阻塞 JS 线程、按键根本到不了
页面，只是在**删除**这条路径上把它盖住了。换成真正的 DOM 浮层之后它才显形。

修法：`src/lib/overlay-layers.ts` 的 `modalLayerOpen()`，在五处让位——
useFocusTrap 的 Tab 处理、SettingsDialog 与 NotificationsPage 的 Esc 监听、
useKeyboardShortcuts 的单键守卫与 Esc 分支。

最后那一处是复审没提、顺着根因自己找到的：确认框从**阅读区**弹出时，
`composeOpen / helpOpen / overlayOpen` 三个布尔量全是 false，于是
「确认要删掉这一封吗」开着的时候按 `#` 删掉的是**另一封**邮件。

判据用 `[role="dialog"][data-state="open"]`：手写的那三个浮层根元素也写着
`role="dialog"`，但 `data-state` 是 radix 自己打的，它们没有，所以不会自己匹配自己。
这一点是 jsdom 探针实测确认的，不是照记忆写的。

#### S4（中高）三条「轮询永不停止」的路径只修了一条

commit message 写着修了「轮询永不停止」，实际只修了「轮询错了账户」那个变体。
原始根因——**phase 永远到不了终态**——还在，而且有三条路径：

| 路径 | 后端行为 | 结果 |
|---|---|---|
| 在停用账户上点同步 | `ErrAccountDisabled` → 500 | 状态记录压根没建立 |
| 同步中删掉账户 | status 端点**不校验账户是否存在**，回 200 `{"phase":"none"}` | 既不是 done 也不是 error |
| 同步中后端重启 | 状态是内存态，重启丢失 → 同上 | 同上 |

三条都是「每秒一个 HTTP 请求打到刷新页面为止，屏幕上零反馈」。

修法：**先等触发成功再开轮询**（这一条同时堵掉了第一条路径）、
`phase === 'none'` 认作终态（`statusStore` 无淘汰，触发成功后再读到 none
...
- **上传进度那条最初照着实现又写了一遍判据**，改实现它也不会红，等于没测。
  抽出导出的 `uploadRatio()` 让测试真正调用它才算数。这是文档里记了三轮的同一个坑。
- **jsdom 对 focus trap 没有见证力**：不排版，`offsetParent` 与
  `getBoundingClientRect()` 全是 0，`focusables()` 恒空，trap 每次都走
  「没有可聚焦元素就吞掉这次 Tab」那一支——修不修都是同一个结果。
  给 rect 打桩之后用例才有区分度。
  而且断言不能看 `defaultPrevented`：radix 自己的 FocusScope 环绕时也会 preventDefault，
  两者混在一起分不清是谁干的（实测 Shift+Tab 就是被 radix 正当拦下的）。
  要看的是用户能感知的那件事——**焦点有没有被抢走**。

顺带修掉一个躺在那里的假 mock：`ComposeDialog.test.tsx` 里
`useToast: () => ({ show, dismiss })`，而真的 `useToast()` 给的是 `{ toast, dismiss }`。
形状对不上不会当场报错，只有真正走到 `toast()` 那一支的用例才会炸，
而那种用例今天一条都没有——于是它可以一直绿着躺着。

### 工具链上的两点

- **`pnpm build`（`tsc -b`）比 `pnpm exec tsc --noEmit` 严**，走 tsconfig.app.json 的
  `noUnusedLocals`。它报出了一个未使用的 import 而后者放行。以后用 build 做判据。
- **`t('common.cancel')` 悬空键**：`common` 命名空间当时根本不存在，正是第一轮
  `t('common.undo')` 的那个坑。第一轮为此加的 `keys.test.ts` 当场抓住了我——
  那条守卫这次真正兑现了价值。

## 六、第六轮已处理（14 条）——把待办清空

一次清掉「待处理」里剩下的全部 14 条（第 10、14~22、25、26、28、29 条）。
逐条对着**当时的代码**核实过，14 条全部属实（前几轮有过清单描述已经过期的情况）。

### 值得单独说的四条

**第 20 条把猜测换成了测量，结论与清单相反。**
清单写的是「`.mail-item` 焦点环 `outline-offset: -2px` 与选中底色混淆，**深色主题**对比不足」。
按 WCAG 1.4.11（非文本对比，门槛 3:1）逐主题算下来，问题**全在亮色主题**：

| 主题 | 环对页面底色 | 环对选中行(accent-wash) |
|---|---|---|
| butter/light | 2.17 | **1.94** |
...
而暗色主题全部在 5.8 以上。六组亮色主题的选中行上键盘焦点基本看不见。
换成同色相的可及调 `--accent-ink`：18 组 × 四种底色最低 4.65。

> 这条的意义不只在于修好了：**「看一眼觉得不够」和「按公式算一遍」会得出相反的结论**，
> 而清单里那句话已经躺了五轮没人质疑过。

**第 25 条顺带挖出一个更实的问题。** 清单说的是「From 下拉带 14 行内联样式而
`.settings-field > select` 是现成的」——但那 14 行里有一条 `outline: 0`，而**内联样式
优先级最高**，把全局那条 `:focus-visible` 焦点环也一起关掉了：Tab 到发件人下拉时
屏幕上毫无反应。抽成 `.compose-select` 之后焦点环自然恢复（那条全局规则用
`:where()` 特异性为 0，所以新类里绝不能再写 `outline`）。

顺带记一笔：**没有复用 `.settings-field > select`**。那是堆叠式表单里的 32px 带框输入，
而这里是「发件人： [下拉]」的行内位置，套上去会在一行文字中间插一个方框。
共用的是令牌与尺度，不是那条规则——"统一"不等于"同一条 CSS"。

**第 22 条我一开始写了句假话。** 初版注释写「不守 NaN 的话会静默落进『更早』分组」，
可 NaN 让三个比较全为 false，**本来就会**落到最后那个 `return t('notif.older')`——
两种写法结果完全一样。改成实话：这一句不修任何行为，留着是因为「靠三次比较都
失败来得到正确答案」是偶然对的，谁把顺序改成从大到小判就会翻车。

（这是第四轮那条教训的复发：**写注释时顺手编一个听起来合理的失败场景，比不写更坏**。
这次是自己在写测试时发现的——想给它写一条会失败的测试，结果发现写不出来。）

**第 28 条是这一轮唯一的新功能。** 正文 iframe 一直强制白底（邮件 HTML 假定白底，
改成深色会让一半邮件白底白字），代价是暗色主题下唯独正文是一块刺眼的白板。
新增「暗化邮件正文」开关：整体 `invert(1) hue-rotate(180deg)` 再把图片/视频反回来。
**默认关闭**且只在暗色模式下生效——反相对用背景图拼版式的营销邮件会显出接缝，
这个代价该由选择打开的人承担。

为此给 `theme.ts` 加了 `getThemeMode` / `subscribeThemeMode`（MutationObserver 盯
`data-mode`）。做成订阅而不是让 `applyTheme` 挨个通知：盯结果比要求每个写入点都
记得广播可靠。两个开关都必须是订阅式的——暗化样式随 `srcDoc` 一次性注入 iframe，
组件不重新渲染就不会重建文档，用户拨了开关而正开着的那封邮件会停在旧样子。

### 其余十条

| 条目 | 做法 |
|---|---|
| 10 | 手搓下拉 → `DropMenu`（radix），净删约 35 行；顺带得到键盘操作、焦点管理、Esc 关闭 |
| 14 | 手风琴折叠行加 `.ti-caret`，与侧栏账户行同一套（尺寸/颜色/旋转/时长）。**刻意不做展开动画**：正文是 iframe，高度要等它加载完再测量回传，对一个还不知道的高度做过渡，得到的是「先展到一个错的高度再跳一下」 |
| 15 | `.attach-card .ac-name` 截断 + `.ac-text{min-width:0}`（flex 子项默认 `min-width:auto`，不置 0 则 ellipsis 永不触发）+ `title` |
| 16 | `.reader-subject` 加 `overflow-wrap: anywhere` |
| 17 | 列表 4 处 `'—'` → `t('list.noSubject')`。更别扭的是行的 `aria-label` 里一直用的是后者，于是读屏念「（无主题）」而屏幕上写着一个破折号 |
| 18 | 附件超过 6 个折叠，`.attach-more` 与引用折叠同一套外观 |
| 19 | 通知筛选 tab 补 `role="tablist"/"tab"/aria-selected` + roving tabindex + 方向键环绕 |
| 21 | 下载改 async + try/catch + toast；>50MB 先确认；下载中 `aria-busy` + 「下载中…」 |
| 26 | `.list-foot` 三种状态都不成立时不渲染。**空的容器不是空的**——它那 12px 上下内边距照常生效，列表底下恒挂一条空白，看起来像「还有一行没加载出来」 |
| 29 | meta 行的重复日期 → 账户（仅多账户时）+ 所属文件夹；`folderLabel` 从 ThreadReader 抽到 `lib/mail-format.ts` 共享，避免判据有两份 |

### 复审抓出的四条（真实浏览器实测）

第六轮同样在提交前跑了独立审查。它用真实 Chromium 量了像素与层叠优先级，
抓出的第一条直接推翻了我对第 28 条的一个核心说法。

**A（高）「图片再反一次回到原样」是错的，而我的测试把这个错误信念一起固化了。**
...
| `#0b57d0` 蓝 | `#125ea6` |
| `#ffd400` 金 | `#e9be87` |
| `#808080` 灰 | `#808080`（无彩色不受影响） |

根因：CSS 的 `hue-rotate` 是一个**固定的线性近似矩阵**而不是真正的色相旋转，
它与 `invert` 的复合**不满足对合律**（单独的 `invert` 才满足）。饱和色每过一轮
掉一截饱和度和明度——照片和品牌 logo 会褪色发暗，金黄那一档掉约 40% 饱和度。

取舍：去掉两处 `hue-rotate` 可让图片逐字节精确还原，代价是正文里**每个蓝链接
都变成橙色**。保留 `hue-rotate`：链接与彩色标题在邮件里出现的频率远高于饱和色图片，
"每封信的链接都变色"是用户一直会撞到的。Dark Reader 的 filter 模式同样如此——
但它没在注释里声称无损，而我声称了。

> 这条的教训不是取舍选错了（取舍本身可辩），而是**我把一个没验证过的"显然"
> 写进了注释，又写了一条只断言 CSS 文本含 `invert(1)` 的测试去"守"它**。
> 那条测试的名字叫「回到原样」——它把错误信念钉死在了测试名里，
> 以后谁读到都会当成已验证的事实。现已改名为「近似还原，不是无损」并列出实测值。

**B（中高）焦点环只换了 `outline:` 那一族，三个拖拽手柄漏了，而我的新测试永远看不见它们。**

`.col-resize` / `.list-col-resize` / `.slide-resize` 的焦点表达是**整条底色**
（3px 宽的分隔条画不下描边），用的是 `background: var(--accent)`。
而 `focusRingTokens()` 的正则只认 `outline:`——**换个 CSS 属性就绕过了整条守卫**。

按第 20 条那张表，butter/light 下 `--accent` 对页面底色只有 2.17:1，
键盘用户 Tab 到列宽手柄后看不出自己在哪——正是第 20 条要解决的那个问题，
只因为换了个属性就漏网。三处已改，正则扩成也扫 `background:` 与 `box-shadow:`。

顺带修掉这条守卫自己的一个缺陷：`themeBlocks().size` 原本断言 `>= 18`，
挡不住"正则少解析到一半"这种退化——少解析的那些主题一条断言都不会跑。收紧成 `=== 18`。

**还有一个更尴尬的：扩大正则之后测试红了，但报的是「`--bg-alt` 对比度 1.00」。**
原因是正则把**注释里**提到的 `:focus-visible` 当成了真规则，`[^{]*` 一路吃到
注释之后下一个无关规则的 `{`。这个仓库的 CSS 注释写得很密，**读 CSS 做断言之前
必须先剥注释**——这是第四轮「vitest 读不到 CSS」之后，同一条路径上的第二个坑。

**C（中）设置页所有下拉框根本没有焦点环——与第 25 条同一个病，只是写在 CSS 里。**

`.settings-field > input, .settings-field > select { outline: none }` 的特异性是
(0,1,1)，而全局 `:where(...):focus-visible` 是 (0,1,0)——前者赢。`select` 明明
在全局名单里却被压掉。实测 Chromium：`.settings-field > select` 聚焦后
`outline-style=none`，而本轮修好的 `.compose-select` 是 `solid / 2px`。

同址还有 `.compose-row input { border: 0; outline: 0 }`——撰写器的收件人/抄送/主题
输入框同样零焦点表达，而且 `input` 压根不在全局 `:where()` 名单里，
所以要**同时**改两处才生效。已清掉两处 `outline`，并把 `input, textarea` 加进全局名单。

**D（中）`[style*="background-image"]` 漏掉 `background: url()` 简写。**
邮件里简写更常见（为兼容 Outlook 通常与 `<td background="…">` 成对写）。
改成 `[style*="url("]` 一次覆盖两种。

审查同时确认了两个覆盖缺口，已如实写进注释而不是继续用"会显出接缝"含糊带过：
写在 `<style>` 块里、靠类选择器套上去的背景图**完全覆盖不到**（服务端净化保留
`<style>`），而 `bgcolor` 属性的底纹跟着整体反相变深，**这正是想要的**，不需要排除。

**另外两条经核实我的说法成立**：第 22 条（NaN 守卫不改变行为）与第 25 条
（内联 `outline: 0` 确实关掉了焦点环，实测 `outlineStyle` 从 `none` 变 `solid`）。

### 复审后半段的五条——其中一条是我修第 20 条时引入的回归

**E（中）焦点环一刀切换成 `--accent-ink`，把暗色从「合格且醒目」改成了「合格但难认」。**

我只算了「环对底色」，没算「环对正文色」。补测之后：

| | 环对底色（最低） | 环对 `--ink` |
|---|---|---|
| 暗色用 `--accent-ink`（我的改法） | 7.85~10.78 ✅ | **1.16~1.54** ⚠ |
| 暗色用 `--accent`（原状） | 5.78~7.94 ✅ | 1.39~2.09 |

slate/dark 的 `--accent-ink` 是 `#d6dbe2`，近白灰——焦点环变成一条与正文同色同亮的
细线。WCAG 仍过，但「强调色 = 焦点」这条辨识线索没了，看起来像一条随机白边。

**亮暗两边的失败方式是相反的**：亮色下 `--accent` 对底色不足，暗色下 `--accent-ink`
对正文色不足。所以新开了 `--focus-ring`，在 `[data-theme][data-mode='light'/'dark']`
两处各定义一次（亮取 `--accent-ink`、暗取 `--accent`）——而不是往 18 个主题块里各塞
一份 hex，那会让色板重新变成两份副本，正是第 24 条修掉的东西。

> 教训不是"漏算了一组数"，而是**我把一个单一维度的达标当成了整件事做对**。
> 3:1 是门槛不是目标；过了门槛之后还有"它看起来像不像焦点"这个问题，
> 而那个问题我连量都没量。

这条的测试用**两条断言**守两种不同的回退，而**都不是** ring/ink 的数值比：

- **结构**：亮暗两边取的令牌必须不同 → 守「又被一刀切回单一令牌」。
- **色相**：环色必须与该主题 `--accent` 同色相（±15°，彩度 <8% 的灰阶主题天然过关）
  → 守「被换成与主题强调色无关的颜色」。调深调浅随意，只要仍满足 3:1。

为什么**不**钉 ring/ink 的对比度（审查给的三条理由，比我自己想的准）：

1. **两个目标在数学上对立。** 亮色主题里 `--ink` 深、底色浅，要让环远离 ink 就得调亮，
   一调亮就靠近底色，3:1 那条立刻掉。实测这个反相关很干净——vs-ink 最高的
   butter/light（2.83）恰好是 vs 底色最低的那组（4.65），而 vs-ink 最低的
   slate/light（1.25）vs 底色高达 10.58。加一条 vs-ink 下限等于和已通过的那条对拉。
2. **门槛无处可放。** 要 18 组全过得 ≤1.25，几乎拦不住任何东西；设 1.3 就只有
   slate/light 失败，而 slate 是**灰阶主题**——它的强调色按定义就是灰，
   永远不可能在明度上远离灰色的正文。那不是缺陷，是主题的设计意图。
3. **真正要守的是色相/彩度而不是明度。** 暗色换回 `--accent` 之后 vs-ink 只有
   1.39~2.09，并不比被换掉的 1.16 高多少，但它**看起来**明显更好，
   因为它是黄的而正文是灰白的——明度比量不到这个差别。

> 这一段本身就是个教训：我原来准备加的那条 vs-ink 断言，**方向是对的、度量是错的**。
> 差一点就把一个量不到目标物的指标钉成守卫，而它还会和已有的断言互相拉扯。

**F（中低）`svg image` / `canvas` 是死代码。** 服务端净化把内联 `<svg>` 连内容一起
整块丢弃（`dropWithContent` 含 svg/math），`<canvas>` 在无脚本沙箱里恒空白。
删掉——"留作将来的预备"是给死代码找借口。

**G（低）`downloadingIdx` 的 `finally` 会清掉另一个下载的忙碌态。**
审查先否掉了我担心的那个风险（硬闸门卡死不成立：`finally` 一定执行、
`key={detail.id}` 重挂载会重置 state），然后指出两个真实的小问题：
①**交错清空**——A（大附件）等确认框时闸门开着，用户点了小附件 B（不走确认框直接下），
再回答 A 的「是」，A 顶掉 B，B 下完后的 `finally` 把忙碌态清成 null，而 A 还在下载；
②`if (downloadingIdx != null)` 读的是闭包快照，**不是真正的互斥**，而注释里
"一次一个"比代码做到的强。闸门改 `useRef`（同步可见），`finally` 只在
`ref.current === idx` 时清空。顺带补了空格键的 `preventDefault()`（既有问题）。

**H（低）tablist 只做了一半。** 补 `role="tabpanel"` + `aria-labelledby` + 各 tab 的 `id`，
加 `Home`/`End`，并让带修饰键的方向键放行——`Ctrl+←` 是很多人的词间移动习惯，
吞掉它就成了"想移动光标结果换了筛选器"。
（审查同时确认 `e.currentTarget.parentElement` 这个 DOM 假设**不脆**：
`currentTarget` 恒为 button 而非点击目标。）

**I（低）空 `DropMenu` 会弹出一个空白浮层。** `moveTargets` 来自
`useFolders(selectionAccountId)` 而**没有按 `selectable` 过滤**，于是「按钮禁不禁用」
看的是未过滤的列表、菜单项用的是过滤后的——全是 `\Noselect` 时按钮可用而菜单为空。
在 Shell 那侧一次过滤，让两处用同一个集合。（不是本轮回归，旧代码同样如此。）

顺带补回一个**我这一轮删掉的能力**：换成 `DropMenu` 时我删了两处
`setBatchMoveOpen(false)`，理由是"radix 自己会管"。radix 确实覆盖了点外部 / Esc /
选中项，但覆盖不到「换数据源」「退出选择模式」这类**由程序**让 trigger 变 disabled
的路径——菜单会留在屏幕上，关闭时焦点回不到已 disabled 的 trigger，掉到 `<body>`。
给 `DropMenu` 加了**可选的**受控 `open`/`onOpenChange`（省略时仍是非受控，
其它调用点零影响），那两处置回 false。

> 删一段代码之前要问的不是"它现在还有用吗"，而是"**它原本在防什么**"。
> 我当时的判断是"radix 都覆盖了"，而那是个没有核对过的概括。

### 本轮的测试

新增 33 条，每条都做了回退验证：

- `focus-ring.test.ts` 4 条（含色相断言）：从 CSS 里把焦点规则**实际用的那个令牌**解析出来再算对比度——
  钉的是结果不是写法，换别的令牌只要达标就照样过。回退成 `--accent` 时它列出全部
  不及格的组合（这也量化了修复前的实际情况）。
- `mail-frame.test.ts` +6 条（暗化正文、背景图两种写法）、
  `mail-format.test.ts` +3 条（`folderLabel`）、
  `NotificationsPage.test.tsx` 8 条（tablist 语义、方向键、tabpanel、Home/End、修饰键让位）、
  `MessageBody.test.tsx` 8 条（附件折叠、下载失败/成功对照、大附件确认、交错下载竞态）、
  `MailList.test.tsx` +4 条（底部空白、加载态、无主题文案、移动按钮禁用）。

三处**测试方法本身**的教训：

1. **写不出会失败的测试，说明那个「修复」没修任何东西。** 第 22 条就是这么暴露的。
2. **mock 的注册顺序会静默吃掉具体 handler。** axios-mock-adapter 按注册顺序匹配，
   `onGet(/.*/)` 放在前面时，后注册的具体 URL 永远命中不到——表现是**测试照样绿**
   （请求被通配拦下返回 200），不是报错。
3. **一条测试可能因为与被测逻辑无关的原因而通过。** 「下载失败会提示」那条最初是绿的，
   但原因是 jsdom 没有 `URL.createObjectURL`——**成功路径也会抛**。补了
   `createObjectURL` 打桩和一条「下载成功时不弹任何提示」的对照用例才算真的测到。
   另外我原以为 `responseType:'blob'` 会让后端的错误文案读不出来，实测在 mock 下
   能读到；如实写进注释，没有把猜测留在那里当事实。

## 七、待处理（0 条）

41 条已全部处理。各条的做法与判据见前六节，编号一律保留原值没有重排。

**本轮之外仍然开着的一件事**（它不在这 41 条里，是做第 7 条时发现的）：
**后台自动同步的进度不可见**。手动点同步现在有完整的进度表达，但 Manager 定时触发的
那一路前端根本不知道它在跑——要补得动后端，给自动同步也发 SSE 事件。

## 八、三条真缺陷的共同形状（2026-09-12 补）
...
**根因三：两套浮层机制互不知情（第五轮补）。**
仓库里有手写浮层（设置面板、通知中心、快捷键速查，键盘处理器装在 `document` 上）
和 radix 浮层（Portal 挂在 `<body>` 末尾）两类。手写那套的判据全是
「焦点在不在我的子树里」「按了 Esc 就关我自己」，对一个**结构上永远在自己之外**
的浮层全部失效——Esc 连着关两层、Tab 被吞、单键快捷键打到背后的列表上。

这条与根因一同形（把"焦点不在我里面"当成"焦点在浮层背后"），
但它有个额外的性质值得单记：**它在第五轮之前就存在，只是被 `window.confirm`
阻塞 JS 线程这件事在删除路径上盖住了**。把一个阻塞式 API 换成非阻塞的，
会让一批本来就坏、只是碰不到的东西同时显形——这类替换要连带复查
「原先那个 API 的副作用替我挡住了什么」。
...
5. ~~**P1 第 7、9、13 条**（长操作无进度表达、设置页 8 处 window.confirm、
   撰写器校验错误显示在会滚走的位置）~~ ——✅ 第五轮完成。
   其中第 7 条做下去才发现不是"缺进度"而是那段代码本来就是坏的。
6. ~~**P1 第 10、14 条 + P2 的一串**（菜单不同源、手风琴无展开指示符，
   以及第 15~22、25、26、28、29 条）~~ ——✅ 第六轮一次清完。
   预判的多是各自独立的小修基本准确，但其中第 20 条按公式一算得出了与清单
   **相反**的结论，第 25 条挖出一个清单没提的焦点环失效，第 28 条是个真功能。
   → 这类锦上添花批次不能真当成扫尾：逐条核实的成本远低于照着旧描述动手。
